...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get active funds for distribution notifications
    const activeFunds = await prisma.fund.findMany({
      where: { userId: session.user.id, isActive: true },
      select: { id: true, name: true, targetPercent: true },
    })
            ...
            if (tx.type === 'incoming') {
              await prisma.notification.create({
                data: {
                  ...
                  message: `На кошелёк "${category.name}" поступило ${tx.amount} USDT`,
                  data: { txId: newTx.id, categoryId: category.id, amount: tx.amount.toString() },
                },
              })

              // Create distribution notification if active funds exist
              if (activeFunds.length > 0) {
                const totalPercent = activeFunds.reduce((sum, f) => sum + f.targetPercent, 0)
                await prisma.notification.create({
                  data: {
                    userId: session.user.id,
                    type: 'distribution_pending',
                    title: 'Распределить поступления',
                    message: `Поступило ${tx.amount} USDT. Рекомендуется распределить ${totalPercent}% по фондам (${activeFunds.map(f => f.name).join(', ')})`,
                    data: { txId: newTx.id, categoryId: category.id, amount: tx.amount.toString(), funds: activeFunds },
                  },
                })
              }
            }
